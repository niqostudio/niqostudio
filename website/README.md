...
問い合わせ系の秘密（`RESEND_API_KEY` / `CONTACT_FROM` / `CONTACT_TO` / `TURNSTILE_SECRET_KEY`・site key）は dev の `.env` に置かず、本番は **wrangler secret**、CI は **Environment** で注入する（未設定なら送信・検証は skip）。`service_role` / `sb_secret_` key は含めない（純フロントには不要）。
