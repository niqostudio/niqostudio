...
- core の `db:reset` は **core の migration しか流さない**＝この studio スキーマは別管理。reset 後は下記 `store:migrate` で作り直す。
...
```sh
pnpm --filter @niqostudio/studio store:migrate   # studio スキーマを作る／更新する
pnpm --filter @niqostudio/studio store:seed       # 妥当な overlay と下書きのダミーを投入
```
