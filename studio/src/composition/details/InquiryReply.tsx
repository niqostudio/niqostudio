...
export async function InquiryReply({ id }: { id: string }) {
  ...
  const [inqRows, replyRows] = await Promise.all([
    metrics.rows('inquiries', ['id', 'email', 'subject', 'message', 'status', 'created_at'], { column: 'id', value: id }).catch(() => []),
    metrics.rows('inquiry_replies', ['id', 'body', 'created_at'], { column: 'inquiry_id', value: id }).catch(() => []),
  ]);
  const inq = inqRows[0];
  if (!inq) return null;
