...
- 本番反映は CI（`saas-platform: migrate`・Environment `saas-platform-production` の承認ゲート）。
...
- **`identity.products` は core.products（is_saas）の射影**（code=slug / name / status のみ）。正本は core
  ...
  反映は `saas-products: sync` workflow（手で行を足さない・消えた code は inactive 化）。
- 製品の追加: ①studio で core に製品（is_saas）・商品を登録 → `saas-products: sync` dispatch
  → ②config.<env>.json の `saas.auth.additional_redirect_urls` に URL 追加（supabase-saas apply）。
