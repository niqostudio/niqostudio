...
| `endpoints.s3` | `https://<account_id>.r2.cloudflarestorage.com`（**account_id から導出**） | Variable `R2_TFSTATE_ACCOUNT_ID` から組み立て |
| `key` | `<stack>/terraform.tfstate`（例 `niqostudio-com/...` `niqo-studio/...`） | workflow が stack ごとに指定 |

- endpoint は account_id から一意に決まるので **account_id だけを持ち**、CI は `backend.tfbackend` 生成時に組み立てる（backend は `init` 前に読まれ HCL 補間が効かないため、導出は workflow / ローカル手書き側で行う）。
...
| スコープ | 権限グループ | アクセス |
| --- | --- | --- |
| Account（NIQO STUDIO） | Cloudflare Pages | **Edit** |
...
> Terraform=`TF_VAR_*`）。**内部用**（state の bucket/endpoint 等）は**配置名に揃える**
> （`R2_TFSTATE_BUCKET` / `R2_TFSTATE_ACCOUNT_ID`）と翻訳が消えて読みやすい。

公開値の CI **Variables**（`R2_TFSTATE_*` / `RESEND_DNS_RECORDS` / `PUBLIC_SUPABASE_*`）の配置は [variables.md](../variables.md) を参照（ここでは再掲しない）。
...
ドメインごとに転置した構成。各ドメインが自分の email / pages / redirect を持ち、Terraform stack（1 stack = 1 ドメイン）と 1:1 で対応する。
...
```jsonc
{
  ...
  "domains": {
    "niqostudio.com": {
      ...
      "pages": {                                 // role ごとのマップ。増えたら additive に追加
        "website": {
          "project_name": "",                    // 空=この role の Pages を管理しない。設定すると箱+ドメイン作成
          "production_branch": "main",
          "subdomain": "",                       // 束ねるサブドメイン（空=apex）。例 "admin"→admin.niqostudio.com
          "cname": ""                            // CNAME 先（空=<project>.pages.dev）
        }
      }
    },
