...
- endpoint は account_id から一意に決まるので **account_id（`CF_ACCOUNT_ID`）だけを持ち**、CI は `backend.tfbackend` 生成時に組み立てる（backend は `init` 前に読まれ HCL 補間が効かないため、導出は workflow / ローカル手書き側で行う）。`CF_ACCOUNT_ID` は Pages deploy（website）とも共有の Repository Variable。
...
> （`R2_TFSTATE_BUCKET`）と翻訳が消えて読みやすい。account_id は Pages deploy と共有のため Repository Variable `CF_ACCOUNT_ID`。
