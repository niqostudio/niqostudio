...
> **責務**：メールの**設計**と **DNS の混ぜ方**（メール系の正本）。コンソール手順は
> [Cloudflare 手順](cloudflare.md) / [Resend 手順](resend.md)、反映と apply 後の検証は
> [デプロイ手順](../deploy.md)、用語は [用語集](glossary.md) を参照。
...
## ⚠️ SPF は apex に1本だけ（重複に注意）

Cloudflare は Email Routing 有効化時に apex へ自動で
`v=spf1 include:_spf.mx.cloudflare.net ~all` を入れる。**Terraform も apex SPF を入れる**ので、
放置すると apex に SPF が2本 → RFC 7208 違反で SPF 全体が無効になる。**自動で入った SPF を消し、
Terraform 版に一本化する**（削除手順は [Cloudflare 手順](cloudflare.md)）。apex SPF は
