...
- プロジェクト作成・SMTP パスワード・ES256 署名鍵はダッシュボード管理（`supabase_project` リソースは
  DB パスワードを state に残すため使わない＝秘密を state に残さない規約）。SMTP のその他設定と
  メールテンプレートは `stacks/supabase-saas` で宣言管理する（partial 更新で smtp_pass に触れない）。
