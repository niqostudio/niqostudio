...
NIQO STUDIO の DB 運用手順（core / studio 両スキーマ＝スキーマ / RLS / migration）。ローカルスタックと本番プロジェクトは infra の Supabase。
...
`db:reset` は「supabase クリーンスタック → `dbmate up`（core / studio）→ seed」を一手で行う（`scripts/db-reset.mjs`）。migration の適用は **dbmate**（`core/migrations`→`public.core_migrations`、`studio/migrations`→`public.studio_migrations` で追跡）。supabase はローカルスタックと型生成のみ（infra 管轄）。ツール選定は [ADR 0004](adr/0004-migration-tooling.md)。
...
本番 DB への反映は **CI（`release` workflow の core_db job）** で行う（本番反映の単一入口・[デプロイ手順](deploy.md)）。

- 実適用ジョブは **Environment `infra-production`（承認ゲート付き）** で実行する（supabase は infra 所有のため infra 系 job と同じ Environment を借りる・[ADR 0005](adr/0005-supabase-into-infra-platform.md)）。接続は同 Environment の Secret **`SUPABASE_DB_URL`**（env `DATABASE_URL` へ渡す）。**CI は IPv4 のみ**なので **Session pooler** の接続文字列を使う（直結 `db.<ref>.supabase.co` は IPv6 で CI からは不達）。
- 適用：ワークフローは core / studio を順に `pnpm exec dbmate --migrations-dir <ns>/migrations --migrations-table <ns>_migrations up`（未適用の前進 migration のみ・`public.<ns>_migrations` で追跡）。
- 既存 DB に dbmate を導入する初回だけは、baseline を「適用済み」として台帳に記録（stamp）してから `up` する（baseline の `create table` を既存スキーマへ再実行させないため）。これは移行時一度きりの作業で、反映の定常運用には含めない。
...
- 型の正本は **migrations**（live プロジェクトでなく local の適用結果を introspection）。**CI（`verify` の db_check job）が migrations と生成型のドリフト検知と適用済み migration の改竄拒否を行う**ので、スキーマ変更時は再生成してコミットする。

## keep-alive

- Free Tier の7日自動停止対策（`profile` への定期 ping）は **`.github/workflows/keep-alive.yml`**（infra 管轄）が担当（core モジュールでは管理しない）。
- 必要な変数（公開値の Variables）の命名・配置は [変数の配置](variables.md) を正本とする。

## saas-platform（niqostudio-saas プロジェクト）

SaaS 共通アカウント基盤は**別 Supabase プロジェクト**（[ADR 0007](adr/0007-saas-identity-project.md)）。上記と同型のパイプラインを別系統で持つ。

| 項目 | core / studio | saas-platform |
|---|---|---|
| ローカルスタック | `pnpm db:start`（54xxx） | `pnpm saas:start`（55xxx・並走可） |
| リセット＋適用＋seed | `pnpm db:reset` | `pnpm saas:reset` |
| migrations | `core/` `studio/` → `public.<ns>_migrations` | `saas-platform/migrations` → `public.saas_platform_migrations` |
| 型生成 | `pnpm db:types` → `database.ts` | `pnpm db:types:saas` → `saas-database.ts` |
| PR チェック | `verify`（db_check） | `verify`（saas_check） |
| 本番適用 | `release` core_db job（`infra-production`） | `release` saas job（`saas-platform-production`・Edge Functions deploy も同 job） |
