...
| 💰 財務 | `invoices` | 不可（請求・入金消込・内部・service_role 専用） |
...
erDiagram
    ...
    projects     ||--o{ invoices          : "project_id (任意)"
    projects     ||--|| ndas              : "project_id (1:1)"
    ndas         ||--o{ nda_events       : "nda_id"
...
| `label` | text NOT NULL | |
| `achieved` | text | 結果（フロントは After）。事前設計では null 可（before＋goal を先に保存）。公開 view は null を除外 |
...
| `pdf_url` | text | 保存した NDA PDF の URL（任意） |
| `agreed_on` | date | 合意日 |
...
### nda_events（NDA 同意の変更履歴・append-only）
NDA の status・公開フラグ・合意日のいずれかが変わるたび現状態をスナップショットする append-only 履歴（「いつ何に同意/変更したか」の記録）。AFTER トリガが記録し、既存 NDA は移行時に1行 backfill。内部のみ。

| 列 | 型 | 備考 |
|---|---|---|
| `nda_id` | uuid FK→ndas | NOT NULL・CASCADE |
| `status` | text NOT NULL | スナップショット時の status |
| `publish_problems` / `publish_deliverables` / `publish_metrics` / `publish_testimonial` | boolean NOT NULL | 公開可否のスナップショット |
| `agreed_on` | date | 合意日のスナップショット |

※ `changed_at`（記録時刻）のみ（append-only）。
...
| `metrics` | product、または project かつ `ndas.publish_metrics` のとき、選択分の集約 `[{label,achieved,previous,unit,kind}]`（goal 除外・**achieved が null の指標は除外**。他は `[]`） |
...
### invoices（財務・請求書）
受託の請求・入金消込を studio で確認/編集するための最小の正本。会計・確定申告・税の正本ではない（外部 / 範囲外）。顧客必須・案件任意（着手金/分割は同一案件に複数行、保守等は案件なしで顧客直）。`total` は保存せず `subtotal + tax` で導出。期日超過も status=sent かつ未入金かつ `due_on` 経過で導出（保存しない）。状態の不変条件は CHECK で担保（`paid`⇒`paid_on` / `sent`・`paid`⇒`issued_on` / `due_on`≥`issued_on`）。源泉徴収（`withholding`）と実入金額（`paid_amount`）を別持ちし、請求額≠入金額に対応。内部のみ。

| 列 | 型 | 備考 |
|---|---|---|
| `client_id` | uuid FK→clients | NOT NULL・ON DELETE RESTRICT（財務記録を守る） |
| `project_id` | uuid FK→projects | 任意・ON DELETE SET NULL |
| `invoice_no` | text | 請求書番号（非 null のみ部分 UNIQUE） |
| `title` | text | 摘要 |
| `subtotal` | numeric NOT NULL | 小計（税抜・CHECK >= 0・既定 0） |
| `tax` | numeric NOT NULL | 消費税（CHECK >= 0・既定 0） |
| `withholding` | numeric NOT NULL | 源泉徴収額（CHECK >= 0・既定 0。入金予定＝`subtotal`+`tax`−`withholding`） |
| `status` | text NOT NULL | draft / sent / paid / void（既定 draft） |
| `issued_on` / `due_on` / `paid_on` | date | 発行日 / 支払期日 / 入金日 |
